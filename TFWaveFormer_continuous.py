"""
TFWaveFormer with Continuous Adaptive Wavelet Scales

K learnable continuous kernel sizes, interpolated between integer convolutions.
Same multi-scale fusion structure as original WaveletFilter.
"""
import numpy as np
import torch
...
class ContinuousWaveletFilter(nn.Module):
    def __init__(self, d_model: int, K: int = 4, device: str = 'cpu'):
        ...
        self.base_scales = list(range(3, 16, 2))
        self.base_convs = nn.ModuleDict({
            str(k): nn.Conv1d(d_model, d_model, kernel_size=k, padding=k // 2, groups=d_model)
            for k in self.base_scales
        })
    ...
    def forward(self, x: torch.Tensor, training: bool = True):
        ...
        for k in range(self.K):
            ...
            lo = int(np.floor(s.item())); hi = int(np.ceil(s.item()))
            lo = max(3, min(15, lo + (lo % 2 == 0)))
            hi = max(3, min(15, hi + (hi % 2 == 0)))
            alpha = s.item() - lo if lo != hi else 0.0
            if lo == hi:
                outputs.append(self.base_convs[str(lo)](x))
            else:
                outputs.append((1 - alpha) * self.base_convs[str(lo)](x) + alpha * self.base_convs[str(hi)](x))
...
class AdaptiveWaveletTransformerLayer(nn.Module):
    def __init__(self, d_model, nhead, max_seq_len, dropout=0.1, device='cpu'):
        super().__init__()
        self.wavelet_filter = ContinuousWaveletFilter(d_model, device=device)
...
class TFWaveFormerContinuous(nn.Module):
